...
models = []
model_names = ['Random Forest', 'Linear Regression', 'Cat Boost', 'XG Boost']
...
def predict_and_display(df_pred):
    ...
    for model_name, prediction in results:
         st.write(f"{model_name} prediction: {prediction}" "mm")
...
humidity = st.sidebar.slider("Humidity", 44, 100, 44)  # Sử dụng danh sách các giá trị từ 44 đến 100
